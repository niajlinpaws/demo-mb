...
class CustomerConnectionHandler extends ChatConnectionHandler {
  ...
  _gotCustomerInput (utterance) {
    ...
    this.router.customerStore
      .getOrCreateCustomer({ customerId: this.socket.id })
      .then(customer => {
        // Tell the router to perform any next steps
        /*var context = '';
        if(this.socket.handshake.query.('context')){
          console.log("Context Has", this.socket.handshake.context);
          context = this.socket.handshake.query.context;
        } else {
          context = null;
          console.log("Context NO", context);
        }*/
        
        this.router.messageStore.createMessage({
          utterance,
          customerId: this.socket.id,
          isAgentResponse: true
        });

        return this.router._routeCustomer(utterance, customer, this.socket.id, this.socket.handshake.query.context);
      })
      .then(response => {
