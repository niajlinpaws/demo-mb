...
class OperatorConnectionHandler extends ChatConnectionHandler {
  ...
  init (operatorId) {
    ...
    this.router.customerStore.getOrCreateCustomer({})
    .then(customerData=>{

      new Promise(resolve=>{
        resolve(customerData.forEach(customer=>{
          return this.router._sendConnectionStatusToOperator(customer.customerId);
        }));
      }).then(()=>{

        this.router.messageStore
        .getMessages(customerData.map(customer=>customer.customerId))
        .then(messages=>{
          return messages.forEach(message => {
            ...
            {
              this.socket.emit(AppConstants.EVENT_OPERATOR_MESSAGE,message);
            }else{
              this.socket.emit(AppConstants.EVENT_CUSTOMER_MESSAGE,message);
            }
          });
        }).catch(error=>{
          console.log('Error operator initialisation messages: ', error);
  ...
  _gotOperatorInput (message) {
    ...
    // save operator input on persistent data store
    this.router.messageStore.createMessage({ ...message, sender: 'operator' });
    ...
    this.router.customerStore
      ...
      .then(customer => {
        ...
        // here, customer mode check is disabled, so that agent and an operator both can reply.
        ...
        return this.router._relayOperatorMessage(message)
          ...
          .then(() => {
            ...
            console.log("after customerConnection ======>\n\n\n\n",customerConnection);
